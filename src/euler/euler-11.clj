...
(defn move-in-direction [[x y] direction]
  "Adjust the coordinates in the given direction."
  (let [dir (direction directions)
        dx (first dir)
        dy (last dir)]
    [(+ dx x)
     (+ dy y)]))

(defn values-in-direction [start-xy direction]
  (loop [xy start-xy
         result []]
    (if (= 4 (count result))
      (remove nil? result)
      (recur (move-in-direction xy direction)
             (conj result (value-at xy))))))

(defn values-in-all-directions [xy]
  (map #(values-in-direction xy %) (keys directions)))
